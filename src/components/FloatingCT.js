...
export default function FloatingCT({ onBrochureClick, onWhatsAppClick, isWhatsAppOpen }) {
  return (
    <>
      ...
      >
        {/* WhatsApp - Mobile (hide when popup is open) */}
      {!isWhatsAppOpen && (
  onWhatsAppClick ? (
    <button onClick={onWhatsAppClick} className="whatsapp-chat sm:hidden w-12 h-12 rounded-xl bg-[#25D366] flex items-center justify-center shadow-[0_12px_30px_rgba(0,0,0,0.25)]">
    ...
    <a href="https://wa.me/918660200662" target="_blank" rel="noopener noreferrer"
      ...
      <img src="/images/whatsapp.svg" alt="whatsapp" className="w-7 h-7" />
        ...
        {/* WhatsApp - Desktop (hide when popup is open) */}
      {/* WhatsApp - Mobile */}
{!isWhatsAppOpen && (
  onWhatsAppClick ? (
