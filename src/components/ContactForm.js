  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      try {
        ...
        // Reset + navigate to thankyou page
        resetForm();
        ...
        navigate("/thankyou", {
          state: {
            ...
            phone: values.phone,
            project: values.inquiry_project,
          },
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:3001/contact", {
        ...
        body: JSON.stringify(formik.values), // Send all form data including inquiry_project and agreeToTerms
      });

      const data = await response.text(); // Response from the API
      if (response.ok) {
        alert(data); // Show success message from API
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  value={formik.values.inquiry_project}
              ...
              {formik.touched.inquiry_project &&
                formik.errors.inquiry_project && (
                  <div className="text-red-300 text-xs mt-1">
                    {formik.errors.inquiry_project}
                  </div>
                )}
