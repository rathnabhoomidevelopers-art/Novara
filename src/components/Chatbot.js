...
// ─── CONFIG ────────────────────────────────────────────────────────────────
// Update these to match your .env / backend settings
const API_ENDPOINT = "/api/chat";
const ASSISTANT_NAME = "Novara";

// Generates a stable session ID per browser tab (persisted in sessionStorage)
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      role: "bot",
      text: `Hi! I'm ${ASSISTANT_NAME}. How can I help you today?`,
      ts: Date.now(),
    },
  ]);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: Date.now() + 1,
          role: "bot",
          text: "Error talking to server. Please try again.",
          ts: Date.now(),
        },
      ]);
    } finally {
