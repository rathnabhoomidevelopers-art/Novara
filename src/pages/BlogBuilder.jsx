    ...
    return (
      <div className="overflow-x-auto rounded-xl border border-[#1A614F]">
        <table className="w-full text-[14px] text-slate-700 border-collapse">
          <thead><tr>{(s.headers || []).map((h, i) => <th key={i} className="text-left px-4 py-3 font-bold border border-[#1A614F]" style={{ background: s.themed ? "#e8dfa8" : "#fff" }}>{h}</th>)}</tr></thead>
          <tbody>{(s.rows || []).map((r, ri) => <tr key={ri} style={{ background: s.themed ? (ri % 2 ? "#f5f0d8" : "#faf7ec") : "#fff" }}>{r.map((c, ci) => <td key={ci} className="px-4 py-2.5 border border-[#1A614F]">{c}</td>)}</tr>)}</tbody>
        </table>
      </div>
    );
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-y-auto" style={{ background: "#F4F1E8" }}>
        ...
        <button onClick={() => onSelect(null)}
          ...
          <div className="w-full mb-8 flex items-center gap-4 p-5 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 cursor-not-allowed">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 bg-slate-200"><Plus size={22} className="text-slate-400" /></div>
            <div><div className="font-bold text-slate-400 text-base">Create new blog</div><div className="text-sm text-slate-400 mt-0.5">Viewers cannot create blogs</div></div>
          </div>
        )}
...
function BlogEditor({ editingBlog, onBack }) {
  const { user, logout, canEdit, canPublish, isAdmin, canManageUsers, canManageRedirects } = useAuth();
            ...
            <div className="flex-1 min-w-0 flex flex-col h-full overflow-hidden">
              ...
              {!canEdit && (
                <div className="mb-4 flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#FBF1D2] border border-[#E3A600]/40 text-[13px] text-[#9a6b00] font-semibold">
                  <ViewIcon size={15}/> You have <strong>view-only</strong> access — contact an admin for edit permissions.
                </div>
              )}
              {/* Permalink / slug */}
